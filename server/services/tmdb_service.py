...
class TMDBService:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    continue
                raise TMDBConnectionError(str(e) if str(e) else "Connection refused")
            except httpx.TimeoutException:
                raise TMDBTimeoutError(endpoint)
            except httpx.RequestError as e:
                raise TMDBConnectionError(str(e) if str(e) else type(e).__name__)

        raise TMDBConnectionError(str(last_error) if str(last_error) else "Max retries exceeded")
    ...
    async def get_cookie_status(self):
        """Get cookie configuration status.

        Returns CookieStatus. The cookie auth path is optional when
        API token is configured.
        """
        from server.models.config import CookieStatus
        return CookieStatus(is_configured=False)
