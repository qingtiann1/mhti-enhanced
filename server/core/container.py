...
def get_scraper_service():
    ...
    if not container.has(Services.SCRAPER):
        # Read Bangumi access token from environment for R18 content access
        bangumi_token = os.environ.get("BANGUMI_ACCESS_TOKEN", "")
        # Read Hanime API URL from environment
        hanime_api_url = os.environ.get("HANIME_API_URL", "")
        container.register_instance(
            Services.SCRAPER,
            ScraperService(
                config_service=get_config_service(),
